...
function Install-TS {
    ...
    Ok "Tailscale installed: $(& (TailscalePath) version | Select-Object -First 1)"
    return $true
}
function Ensure-TS {
    Refresh-Path
...
function Join-IfNeeded($p) {
    ...
    try {
        & $p up --auth-key=$key --accept-dns=false
        if ($LASTEXITCODE -ne 0) { Warn 'Tailscale authentication failed.'; return $false }
    } finally { $key = $null }
...
function Setup-Exit($p) {
    ...
    # Tags are an authentication identity. They cannot be added with
    # `tailscale set`, and an auth-key-authenticated node must be re-authenticated
    # with an auth key that already permits/contains the requested tag.
    Info 'Configuring Exit Node identity...'
    $secure = Read-Host 'Tailscale Auth Key (required to apply tag:exit)' -AsSecureString
    $bstr = [Runtime.InteropServices.Marshal]::SecureStringToBSTR($secure)
    try { $key = [Runtime.InteropServices.Marshal]::PtrToStringBSTR($bstr) } finally { [Runtime.InteropServices.Marshal]::ZeroFreeBSTR($bstr) }
    try {
        & $p up --auth-key=$key --accept-dns=false --advertise-exit-node --advertise-tags=tag:exit
        if ($LASTEXITCODE -ne 0) { Warn 'Exit Node authentication/tagging failed. Make sure the auth key is allowed to use tag:exit.'; return }
    } finally { $key = $null }
    ...
    if ($status -match 'tag:exit') { Ok 'Exit Node configured with tag:exit.' }
    else { Warn 'Exit Node is enabled, but tag:exit could not be verified in local status.' }
}
function Setup-SSH($p) {
    if (-not (Join-IfNeeded $p)) { return }
    & $p set --ssh
    if ($LASTEXITCODE -eq 0) { Ok 'Tailscale SSH enabled.' } else { Warn 'This Tailscale build did not accept --ssh on Windows Server.' }
    Warn 'If Tailscale SSH is unavailable, use Windows OpenSSH over the Tailscale IP instead.'
}
...
function Ping-Test($p) {
    ...
    $self=@($d.Self.TailscaleIPs); $direct=0; $derp=0; $total=0
    foreach($peer in $d.Peer.PSObject.Properties.Value){
        $ip=@($peer.TailscaleIPs)[0]
        if(-not $ip -or $self -contains $ip){continue}
        $name=if($peer.HostName){$peer.HostName}else{$peer.DNSName}; $total++
        Write-Host "--- $name ($ip) ---"
        $out=& $p ping --c=3 $ip 2>&1 | Out-String; Write-Host $out
...
function Status($p) {
    if(-not (Join-IfNeeded $p)){return}
    & $p status; Write-Host "`nTailscale IPv4:"; & $p ip -4; Write-Host "Tailscale IPv6:"; & $p ip -6
    Write-Host "`nPublic IP:"; Get-PublicIP
}
function Exit-IP($p) { if($p=Ensure-TS){ Info 'Current public IPs (verify again while a client is using this node as Exit Node):'; Get-PublicIP } }
function Health($p) {
    if(-not (Join-IfNeeded $p)){return}
    ...
    & $p status; Get-PublicIP
}
function MTU-Test($p) {
    if(-not (Join-IfNeeded $p)){return}
    $target=Read-Host 'Peer Tailscale IPv4 (100.x.x.x)'
    if($target -notmatch '^100\.[0-9]+\.[0-9]+\.[0-9]+$'){Warn 'Invalid Tailscale IPv4.';return}
    foreach($size in 1472,1400,1300,1200){$r=ping.exe -n 2 -f -l $size $target 2>&1;if($LASTEXITCODE -eq 0){Ok "Payload $size bytes: OK"}else{Warn "Payload $size bytes: failed"}}
}
function DNS-Test {
    Info '=== DNS Test ==='; Get-DnsClientServerAddress -AddressFamily IPv4,IPv6 | Format-Table -AutoSize
    foreach($h in 'controlplane.tailscale.com','tailscale.com'){try{Resolve-DnsName $h -ErrorAction Stop | Out-Null; Ok "$h resolves"}catch{Warn "$h does not resolve"}}
}
function Full-Benchmark($p) {
    if(-not (Join-IfNeeded $p)){return}; Info '=== VPS Quick Benchmark ==='; "Hostname    : $env:COMPUTERNAME"; Get-PublicIP
    "Tailscale IPv4: $(& $p ip -4)"; "Tailscale IPv6: $(& $p ip -6)"; Write-Host '`nNetwork'; & $p netcheck
    Write-Host '`nPeer connectivity'; Ping-Test $p; Write-Host '`nExit Node health'; Health $p
}
...
function Uninstall-TS($p) {
    ...
    Ok 'Uninstall requested. Verify Programs/Apps if the installer remains.'
}
