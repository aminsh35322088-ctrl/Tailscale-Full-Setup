...
ensure_ts(){
  if ! installed; then install_ts || return 1; else resolve_ts; service_up; fi
}
...
join_if_needed(){
  ...
  if ! "$TS_BIN" up --auth-key="$key" --accept-dns=false; then
    unset key; fail "Tailscale join failed. Check the auth key and network connectivity."; return 1
  fi
  unset key
...
has_exit_tag(){
  "$TS_BIN" status --json 2>/dev/null | grep -q 'tag:exit'
}

apply_exit_tag(){
  # Tags are authentication identity. With an auth-key authenticated node,
  # advertise-tags must be supplied during authentication/re-authentication;
  # it is not a mutable `tailscale set` preference.
  echo
  read -r -s -p "Tailscale Auth Key (required to apply tag:exit): " key
  echo
  [[ -n "$key" ]] || { warn "Auth key is empty."; return 1; }

  "$TS_BIN" logout >/dev/null 2>&1 || true
  if ! "$TS_BIN" up --auth-key="$key" --advertise-tags=tag:exit --advertise-exit-node --accept-dns=false; then
    unset key
    fail "Could not authenticate with tag:exit and advertise the Exit Node."
    fail "Make sure the auth key is allowed to use tag:exit and tagOwners permits it."
    return 1
  fi
  unset key

  if ! has_exit_tag; then
    fail "Tailscale authenticated successfully, but tag:exit is not present in local state."
    return 1
  fi
...
setup_exit(){
  ...
  if ! has_exit_tag; then
    warn "This node is connected but is not tagged tag:exit. Re-authentication is required to apply the tag."
    apply_exit_tag || return 1
  fi

  if ! "$TS_BIN" set --advertise-exit-node=true; then
    fail "Could not advertise Exit Node."
    return 1
  fi

  if ! "$TS_BIN" status >/dev/null 2>&1; then
    fail "Tailscale status check failed after Exit Node configuration."
    return 1
  fi
...
get_peers(){
  if ! command -v python3 >/dev/null 2>&1; then warn "python3 is required for peer discovery."; return 0; fi
...
ping_test(){
  ...
  while read -r ip name; do
    [[ -n "$ip" ]] || continue
    count=$((count+1)); echo "--- $name ($ip) ---"
    local out; out="$("$TS_BIN" ping --c=3 "$ip" 2>&1 || true)"; echo "$out"
    if grep -q 'via DERP' <<<"$out"; then derp=$((derp+1)); elif grep -Eq 'via [0-9a-fA-F:]+:[0-9]+' <<<"$out"; then direct=$((direct+1)); else failed=$((failed+1)); fi
  done < <(get_peers)
  echo; [[ $count -gt 0 ]] || warn "No other Tailnet peers found."; info "Summary: $direct Direct / $derp DERP / $failed Failed / $count peers"
}
...
public_ip(){
  ...
  local v4 v6; v4="$(curl -4 -fsS --max-time 5 https://api.ipify.org 2>/dev/null || true)"; v6="$(curl -6 -fsS --max-time 5 https://api64.ipify.org 2>/dev/null || true)"
  echo "Public IPv4 : ${v4:-Unavailable}"; echo "Public IPv6 : ${v6:-Unavailable}"
}
location_info(){
  command -v curl >/dev/null 2>&1 || return 0
  local json country city region; json="$(curl -4 -fsS --max-time 5 https://ipinfo.io/json 2>/dev/null || true)"
  if [[ -n "$json" ]] && command -v python3 >/dev/null 2>&1; then read -r country region city < <(printf '%s' "$json" | python3 -c 'import json,sys; d=json.load(sys.stdin); print(d.get("country","-"),d.get("region","-"),d.get("city","-"))' 2>/dev/null || true); echo "Location    : ${city:--}, ${region:--}, ${country:--}"; else echo "Location    : Unavailable"; fi
}
exit_ip_test(){ ensure_ts || return 1; public_ip; }
health(){
  join_if_needed || return 1; echo; info "=== Exit Node Health ==="
  ...
  "$TS_BIN" status >/dev/null 2>&1 && ok "Tailnet connection: active" || warn "Tailnet connection: unavailable"
  has_exit_tag && ok "tag:exit: present" || warn "tag:exit: not present"
  echo; public_ip
}
...
menu(){
  ...
  echo -e "${CYAN}╚══════════════════════════════════════════╝${NC}"
  echo; echo "  1) Install Tailscale"; echo "  2) Setup Exit Node (IPv4 + IPv6)"; echo "  3) Enable Tailscale SSH"; echo "  4) Ping / Direct-DERP Test"; echo "  5) Network Benchmark (netcheck)"; echo "  6) Show Status + Public IP + Location"; echo "  7) Logout / Remove This Device"; echo "  8) Exit IP Test"; echo "  9) Exit Node Health Check"; echo " 10) MTU Test"; echo " 11) DNS Test"; echo " 12) Full VPS Benchmark"; echo " 13) Uninstall Tailscale"; echo "  0) Exit"; echo
  read -r -p "Select an option: " choice
  case "$choice" in 1) install_ts ;; 2) setup_exit ;; 3) setup_ssh ;; 4) ping_test ;; 5) netcheck ;; 6) status ;; 7) remove_node ;; 8) exit_ip_test ;; 9) health ;; 10) mtu_test ;; 11) dns_test ;; 12) benchmark ;; 13) uninstall_ts ;; 0) exit 0 ;; *) warn "Invalid option" ;; esac
  echo; read -r -p "Press Enter to return to menu..." _; menu
}
require_root || exit 1
menu
